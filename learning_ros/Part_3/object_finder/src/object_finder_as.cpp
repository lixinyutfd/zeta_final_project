...
bool ObjectFinder::find_toy_block(float surface_height, geometry_msgs::PoseStamped &object_pose) {
    ...
    Eigen::Vector3f major_axis_2;
    Eigen::Vector3f centroid_2;
    Eigen::Vector3f centroid;
    Eigen::Vector3f centroid_3;

    bool found_object = false; //should verify this
    ...
    cube_found = pclUtils_.find_plane_fit(0.4, 1, -0.5, 0.5, surface_height + 0.025, surface_height + 0.045, 0.001,
        plane_normal, plane_dist, major_axis_2, centroid_2);
    ROS_INFO("2%f",centroid_2(0));
    toy_found = pclUtils_.find_plane_fit(0.4, 1, -0.5, 0.5, surface_height + 0.060, surface_height + 0.080, 0.001,
            plane_normal, plane_dist, major_axis_2, centroid_3); 
    ROS_INFO("3%f",centroid_3(0));
    if(cube_found && toy_found) {
        ROS_INFO("Both_found! It is a toy_bock and a cube!");
        ROS_INFO("lalala");
        found_object = true;
        toy_found = pclUtils_.find_plane_fit(0.4, 1, -0.5, 0.5, surface_height + 0.025, surface_height + 0.045, 0.001,
        plane_normal, plane_dist, major_axis, centroid);
        ROS_INFO("0%f",centroid(0));
    }
    else if(cube_found) {
        ...
        found_object = true;
        object_id_found = 0;
        toy_found = pclUtils_.find_plane_fit(0.4, 1, -0.5, 0.5, surface_height + 0.060, surface_height + 0.085, 0.001,
        plane_normal, plane_dist, major_axis, centroid);
        ROS_INFO("0%f",centroid(0));
    }
     ...
     ROS_INFO("object_id = %d", object_id_found);
...
void ObjectFinder::executeCB(const actionlib::SimpleActionServer<object_finder::objectFinderAction>::GoalConstPtr& goal) {
    ...
    if (!known_surface_ht) {
        ...
        table_ht = pclUtils_.find_table_height(0.5, 1, -0.5, 0.5, -0.5, 0.5, 0.005);
    ...
    switch (object_id) {
        ...
        case ObjectIdCodes::TOY_BLOCK_ID:
            ...
            if (found_object) {
                ...
                result_.found_object_code = object_finder::objectFinderResult::OBJECT_FOUND;
                result_.object_pose = object_pose;
                result_.object_id = object_id_found;
                object_finder_as_.setSucceeded(result_);
            } else {
                ROS_WARN("could not find requested object");
